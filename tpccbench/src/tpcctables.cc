...
TPCCTables::~TPCCTables()
{
    ...
    // in the general case:
    // call the destructor for warehouses, stock etc
}
                              ...
                              NewOrderOutput *output, TPCCUndo **undo)
{
    ...
    // the save inserts the district in the "modified districts" set -
    // so we know to undo the modification if the transaction needs to be rolled back
    if (undo != NULL)
    {
        (*undo)->save(d);
    }
...
template <typename T>
static T *insert(TreeApi *tree, int32_t key, const T &item)
{
    assert(!tree->find(reinterpret_cast<void *>(&key)));
    T *copy = new T(item);
    // T *copy2 = NULL;
    tree->insert(reinterpret_cast<void *>(&key), &copy);
    // tree->find(reinterpret_cast<void *>(&key), &copy2);
    // assert(*copy2 != *copy);
    return copy;
}

// For example TreeApi<int32_t, Order*>
template <typename T>
// actually, might be better to use a template for the key too instead of int32_t
// pass the rettype as a parameter, not the smartest design choice hmmm
static T *find(const TreeApi *tree, int32_t key)
{
    T *output = NULL;
    if (tree->find(reinterpret_cast<void *>(&key), &output))
    {
        return output;
    }
    return NULL;
...
static void erase(TreeApi *tree,
                  KeyType key, const T *value)
{
    T *out = NULL;
    ASSERT(tree->find(reinterpret_cast<void *>(&key), &out));
    ASSERT(out == value);
    bool result = tree->remove(reinterpret_cast<void *>(&key));
    ASSERT(result);
    ASSERT(!tree->find(reinterpret_cast<void *>(&key)));
}
...
{
    return find<Warehouse>(warehouses_, id);
}
...
{
    return find<Stock>(stock_, makeStockKey(w_id, s_id));
}
...
{
    return find<District>(districts_, makeDistrictKey(w_id, d_id));
}
...
static int32_t makeCustomerKey(int32_t w_id, int32_t d_id, int32_t c_id)
{
    ...
    int32_t id = (w_id * District::NUM_PER_WAREHOUSE + d_id) * Customer::NUM_PER_DISTRICT + c_id;
    assert(id >= 0);
    return id;
}
...
{
    return find<Customer>(customers_, makeCustomerKey(w_id, d_id, c_id));
}
...
static int32_t makeOrderKey(int32_t w_id, int32_t d_id, int32_t o_id)
{
    ...
    int32_t id = (o_id * District::NUM_PER_WAREHOUSE + d_id) * Warehouse::MAX_WAREHOUSE_ID + w_id;
    assert(id >= 0);
    return id;
}
...
static int64_t makeOrderByCustomerKey(int32_t w_id, int32_t d_id, int32_t c_id, int32_t o_id)
{
    ...
    assert(1 <= c_id && c_id <= Customer::NUM_PER_DISTRICT);
    assert(1 <= o_id && o_id <= Order::MAX_ORDER_ID);
    // this is like a row major calculation of the offset where the dimensions would be
    // n1: warehouse, n2: district, n3: customer
    int32_t top_id = (w_id * District::NUM_PER_WAREHOUSE + d_id) * Customer::NUM_PER_DISTRICT + c_id;
    assert(top_id >= 0);
    // why shift by 32? 64-bit architecture, store the w-d-c id in 32 msb and o_id in 32 lsb
...
Order *TPCCTables::insertOrder(const Order &order)
{
    ...
    assert(!orders_by_customer_->find(reinterpret_cast<void *>(&key)));
    orders_by_customer_->insert(reinterpret_cast<void *>(&key), &tuple);
    return tuple;
}
Order *TPCCTables::findOrder(int32_t w_id, int32_t d_id, int32_t o_id)
{
    return find<Order>(orders_, makeOrderKey(w_id, d_id, o_id));
}
Order *TPCCTables::findLastOrderByCustomer(const int32_t w_id, const int32_t d_id, const int32_t c_id)
{
    ...
    key += ((int64_t)1) << 32;
    ASSERT(key > 0);
    // need to remove findLastLessThan, it is bplustree specific (replace with what?)
    // why do you need findLastLessThan?
    // or also add a findLastLessThan to the tree_api
    bool found = orders_by_customer_->findLastLessThan(reinterpret_cast<void *>(&key), &order);
...
static int32_t makeOrderLineKey(int32_t w_id, int32_t d_id, int32_t o_id, int32_t number)
{
    ...
    int32_t id = ((o_id * District::NUM_PER_WAREHOUSE + d_id) * Warehouse::MAX_WAREHOUSE_ID + w_id) * Order::MAX_OL_CNT + number;
    assert(id >= 0);
    return id;
}
...
{
    return find<OrderLine>(orderlines_, makeOrderLineKey(w_id, d_id, o_id, number));
}
