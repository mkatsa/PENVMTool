...
class BPlusTree : public TreeApi
// will need definitions for insert, remove, find, and the destructor (tpcctables.cc needs the destructor)
{
public:
        ...
        // bool because tree_api declares it as bool, might change back to void
        // both key and value are values (passed by value)
...
bool find(const void* Nkey, void* Nvalue= nullptr) const {
    KEY key = *const_cast<KEY*>(reinterpret_cast<const KEY *>(Nkey));
    // VALUE is e.g. Order *
...
// I would prefer del to be named remove instead, but instead of renaming it 
// I will just make a wrapper named remove to avoid messing stuff up (at least for now)
...
bool findLastLessThan(const void* Nkey, void* Nvalue = nullptr, void* Nout_key = nullptr) const override {
    ...
    // should be *value ?
    result = findLastLessThanImplement(key, value, out_key);
    return result;
}
...
bool findLastLessThanImplement(const KEY& key, VALUE *value = 0, KEY* out_key = 0) const {
    const void* node = root;
    ...
    if (pos <= leaf->num_keys) {
        ...
        if (pos < leaf->num_keys) {
            ...
            if (leaf->values[pos]) {
                ...
                return findLastLessThanImplement(leaf->keys[pos], value, out_key);
            }
        }
    }
