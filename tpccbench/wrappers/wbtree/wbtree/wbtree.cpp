...
void tpccBenchScan(tree *t, unsigned long prev_key, unsigned long key, unsigned long &retkey, unsigned long &retval)
{
	int loc, i;
	unsigned long search_count = 0;
	struct timespec t1, t2;
	unsigned long elapsed_time;
	node *curr = t->root;
	unsigned long cur_key = prev_key;
	retval = 0;

//	clock_gettime(CLOCK_MONOTONIC, &t1);
	curr = find_leaf_node(curr, prev_key);
	loc = Search(curr, curr->slot, prev_key);

	while (true) {
		for (i = loc; i <= curr->slot[0]; i++) {
			if (curr->entries[curr->slot[i]].key >= key)
				return;
			cur_key = curr->entries[curr->slot[i]].key;
			retkey = cur_key;
			retval = (unsigned long)(curr->entries[curr->slot[i]].ptr);
		}

		curr = curr->leftmostPtr;
		if (curr == NULL) {
			// printf("scanned all keys\n");
			return;
		}
		loc = 1;
	}
