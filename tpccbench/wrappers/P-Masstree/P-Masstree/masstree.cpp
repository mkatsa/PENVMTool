...
void* masstree::tpccBenchScan(uint64_t prev_key, uint64_t end_key, ThreadInfo &threadEpocheInfo)
{
    EpocheGuard epocheGuard(threadEpocheInfo);
    void *root = NULL;
    key_indexed_position kx_;
...
restart:
    ...
    while (p->level() != 0) {
inter_retry:
        next = p->advance_to_key(prev_key);
        if (next != p) {
            p = next;
            goto inter_retry;
        }
        ...
        kx_ = p->key_lower_bound(prev_key);
...
leaf_retry:
        ...
        for (int i = 0; i < perm.size(); i++) {
            snapshot_v = l->value(perm[i]);
            if (l->key(perm[i]) >= prev_key) {
                if (l->key(perm[i]) < end_key && l->key(perm[i]) >= retkey)
                {
                    retval = snapshot_v;
                    retkey = l->key(perm[i]);
                }
