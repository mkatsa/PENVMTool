...
{
    std::cout<<"User Defined :: Operator new [] size:" << sz;
    void* m = malloc(sz);
    std::cout << " address: " << m << std::endl;
    return m;
}
...
#include "clht.h"
#include "ssmem.h"
#include "third-party/CrashTest/wbtree/wbtree.h"
#include "third-party/CrashTest/fptree/FPTree.h"
//#include "FPTree/fptree.h"
//#include "third-party/bztree/bztree.h"
        ...
        std::vector<int> &ops)
{
    ...
    string path_prefix_to_workloads = "./ntua-recipe-ok/index-microbench/";

    if (ap == UNIFORM) {
        if (kt == STRING_KEY && wl == WORKLOAD_A) {
            init_file = path_prefix_to_workloads + "/workloads/ycsbkey_load_workloada";
            txn_file = path_prefix_to_workloads + "/workloads/ycsbkey_run_workloada";
        } else if (kt == STRING_KEY && wl == WORKLOAD_B) {
            init_file = path_prefix_to_workloads + "/workloads/ycsbkey_load_workloadb";
            txn_file = path_prefix_to_workloads + "/workloads/ycsbkey_run_workloadb";
        } else if (kt == STRING_KEY && wl == WORKLOAD_C) {
            init_file = path_prefix_to_workloads + "/workloads/ycsbkey_load_workloadc";
            txn_file = path_prefix_to_workloads + "/workloads/ycsbkey_run_workloadc";
        } else if (kt == STRING_KEY && wl == WORKLOAD_D) {
            init_file = path_prefix_to_workloads + "/workloads/ycsbkey_load_workloadd";
            txn_file = path_prefix_to_workloads + "/workloads/ycsbkey_run_workloadd";
        } else if (kt == STRING_KEY && wl == WORKLOAD_E) {
            init_file = path_prefix_to_workloads + "/workloads/ycsbkey_load_workloade";
            txn_file = path_prefix_to_workloads+ "/workloads/ycsbkey_run_workloade";
        }
    } else {
        if (kt == STRING_KEY && wl == WORKLOAD_A) {
            init_file = path_prefix_to_workloads + "/workloads/ycsbkey_load_workloada";
    ...
    std::cout << "txn_file = " << txn_file << std::endl;
    ...
    if (index_type == TYPE_ART) {
...

        {
            ...
            auto func = [&]() {
                ...
                for (uint64_t i = start_key; i < end_key; i++) {
                    ...
                    //if (i % 5000000 == 0) 
                      //      std::cout << "Load phase: inserted " << i << " keys" << std::endl;
                }
...

        {
            ...
            auto func = [&]() {
                ...
                for (uint64_t i = start_key; i < end_key; i++) {
                    ...
                    //if (i % 5000000 == 0) 
                            ///std::cout << "Run phase: processed " << i << " keys" << std::endl;
                }
                t->UnregisterThread(thread_id);
            };
    ...
    #ifdef STRING_TYPE
        std::cout << "wbtree string type" << std::endl;
        wbtree::tree *tree = wbtree::initTree();

        {
            ...
            //tbb::parallel_for(tbb::blocked_range<uint64_t>(0, LOAD_SIZE), [&](const tbb::blocked_range<uint64_t> &scope) {
            //    auto t = tree->getThreadInfo();
                std::cout << "wbtree load_size = " << LOAD_SIZE << std::endl;
                for (uint64_t i = 0; i < LOAD_SIZE; i++) {
                    //tree->put((char *)init_keys[i]->fkey, init_keys[i]->value, t);
                    //std::cout << "wbtree insert -> " << i  << " " << (unsigned char *)init_keys[i]->fkey << " " << init_keys[i]->key_len << " " << (void *)init_keys[i]->value << std::endl;
                    wbtree::Insert(tree, (unsigned char *)init_keys[i]->fkey, init_keys[i]->key_len, (void *)init_keys[i]->value);
                    //std::cout << "wbtree insert " << i << "done" << std::endl;
...

        {
            ...
            //    auto t = tree->getThreadInfo();
                for (uint64_t i = 0; i < LOAD_SIZE ; i++) {
                    if (ops[i] == OP_INSERT) {
                        wbtree::Insert(tree, (unsigned char *)keys[i]->fkey, keys[i]->key_len, (void*) keys[i]->value);
                    } else if (ops[i] == OP_READ) {
                        wbtree::Lookup(tree, (unsigned char *)keys[i]->fkey, keys[i]->key_len);
                        //uint64_t *ret = reinterpret_cast<uint64_t *> (tree->get((char *)keys[i]->fkey, t));
                        /*if (ap == UNIFORM && (uint64_t) ret != keys[i]->value) {
                            printf("[MASS] search key = %lu, search value = %lu\n", keys[i]->value, ret);
                            exit(1);
                        ...
                        uint64_t results[200];
                        //resultsFound = tree->scan((char *)keys[i]->fkey, ranges[i], results, t);
                        wbtree:Range_Lookup(tree, (unsigned char*)keys[i]->fkey, keys[i]->key_len, ranges[i], results);
                    } else if (ops[i] == OP_DELETE) {
                        //tree->del((char *)keys[i]->fkey, t);
                        wbtree::Delete(tree, (unsigned char*)keys[i]->fkey, keys[i]->key_len);
                    } else if (ops[i] == OP_UPDATE) {
                        //tree->put((char *)keys[i]->fkey, keys[i]->value, t);
                        wbtree::Update(tree, (unsigned char*)keys[i]->fkey, keys[i]->key_len, (void*)keys[i]->value);
                    }
                }
            //});
            auto duration = std::chrono::duration_cast<std::chrono::microseconds>(
    ...
    #ifdef STRING_TYPE
        std::cout << "fptree string type" << std::endl;
        fptree::tree * tree = fptree::initTree();
...

        {
            ...
            std::cout << "fptree loading initial keys" << std::endl;
            auto starttime = std::chrono::system_clock::now();
            tbb::parallel_for(tbb::blocked_range<uint64_t>(0, LOAD_SIZE), [&](const tbb::blocked_range<uint64_t> &scope) {
                ...
                for (uint64_t i = scope.begin(); i != scope.end(); i++) {
                    //tree->put((char *)init_keys[i]->fkey, init_keys[i]->value, t);
                    //std::cout << "wbtree insert -> " << i  << " " << (unsigned char *)init_keys[i]->fkey << " " << init_keys[i]->key_len << " " << (void *)init_keys[i]->value << std::endl;
                    fptree::Insert(tree, (unsigned char *)init_keys[i]->fkey, init_keys[i]->key_len, (void *)init_keys[i]->value);
                    //std::cout << "wbtree insert " << i << "done" << std::endl;
...

        {
            ...
            tbb::parallel_for(tbb::blocked_range<uint64_t>(0, RUN_SIZE), [&](const tbb::blocked_range<uint64_t> &scope) {
                for (uint64_t i = scope.begin(); i != scope.end(); i++) {
                    if (ops[i] == OP_INSERT) {
                        fptree::Insert(tree, (unsigned char *)keys[i]->fkey, keys[i]->key_len, (void*) keys[i]->value);
                    } else if (ops[i] == OP_READ) {
                        fptree::Lookup(tree, (unsigned char *)keys[i]->fkey, keys[i]->key_len);
                        //uint64_t *ret = reinterpret_cast<uint64_t *> (tree->get((char *)keys[i]->fkey, t));
                        /*if (ap == UNIFORM && (uint64_t) ret != keys[i]->value) {
                            printf("[MASS] search key = %lu, search value = %lu\n", keys[i]->value, ret);
                            exit(1);
                        ...
                        uint64_t results[200];
                        //resultsFound = tree->scan((char *)keys[i]->fkey, ranges[i], results, t);
                        fptree:Range_Lookup(tree, (unsigned char*)keys[i]->fkey, keys[i]->key_len, ranges[i], results);
                    } else if (ops[i] == OP_DELETE) {
                        //tree->del((char *)keys[i]->fkey, t);
                        //fptree::Delete(tree, (unsigned char*)keys[i]->fkey, keys[i]->key_len);
                    } else if (ops[i] == OP_UPDATE) {
                        //tree->put((char *)keys[i]->fkey, keys[i]->value, t);
                        fptree::Update(tree, (unsigned char*)keys[i]->fkey, keys[i]->key_len, (void*)keys[i]->value);
                    }
                }
            });
            auto duration = std::chrono::duration_cast<std::chrono::microseconds>(
...
void ycsb_load_run_randint(int index_type, int wl, int kt, int ap, int num_thread,
        ...
        std::vector<int> &ops)
{
    //std::cout << "ycsb_load_run" << std::endl;
    std::string init_file;
    std::string txn_file;

    string prefix_path_to_workloads = "./ntua-recipe-ok/index-microbench";

    if (ap == UNIFORM) {
        if (kt == RANDINT_KEY && wl == WORKLOAD_A) {
            //std::cout << "uniform randint workload a check1" << std::endl;
            //std::cout << "prefix path = " << prefix_path_to_workloads << std::endl;
            //init_file = prefix_path_to_workloads + "/workloads/loada_unif_int.dat";
            //txn_file = prefix_path_to_workloads + "/workloads/txnsa_unif_int.dat";
            init_file = "./ntua-recipe-ok/index-microbench/workloads/loada_unif_int.dat"; 
            txn_file = "./ntua-recipe-ok/index-microbench/workloads/txnsa_unif_int.dat";
            //std::cout << "uniform randint workload a - check2" << std::endl;
        } else if (kt == RANDINT_KEY && wl == WORKLOAD_B) {
            init_file = prefix_path_to_workloads + "/workloads/loadb_unif_int.dat";
            txn_file = prefix_path_to_workloads + "/workloads/txnsb_unif_int.dat";
        } else if (kt == RANDINT_KEY && wl == WORKLOAD_C) {
            init_file = prefix_path_to_workloads + "/workloads/loadc_unif_int.dat";
            txn_file = prefix_path_to_workloads + "/workloads/txnsc_unif_int.dat";
        } else if (kt == RANDINT_KEY && wl == WORKLOAD_D) {
            init_file = prefix_path_to_workloads + "/workloads/loadd_unif_int.dat";
            txn_file = prefix_path_to_workloads + "/workloads/txnsd_unif_int.dat";
        } else if (kt == RANDINT_KEY && wl == WORKLOAD_E) {
            init_file = prefix_path_to_workloads + "/workloads/loade_unif_int.dat";
            txn_file = prefix_path_to_workloads + "/workloads/txnse_unif_int.dat";
        }
    } else {
        if (kt == RANDINT_KEY && wl == WORKLOAD_A) {
            init_file = prefix_path_to_workloads + "/workloads/loada_unif_int.dat";
    ...
    while ((count < LOAD_SIZE) && infile_load.good()) {
        ...
        //if (count % 10000000 == 0) 
          //  std::cout << "loaded " << count << " keys" << std::endl;
    ...
    while ((count < RUN_SIZE) && infile_txn.good()) {
        ...
        //if (count % 10000000 == 0) 
         //   std::cout << "prepared " << count << " operations" << std::endl;
        count++;
    }
    ...
    if (index_type == TYPE_ART) {
        ...
        //std::cout << "bwtree starting" << std::endl;
        //BwTree<uint64_t, uint64_t, KeyComparator, KeyEqualityChecker>* t = (BwTree<uint64_t, uint64_t, KeyComparator, KeyEqualityChecker>*)malloc(sizeof(BwTree<uint64_t, uint64_t, KeyComparator, KeyEqualityChecker>));
        auto t = new BwTree<uint64_t, uint64_t, KeyComparator, KeyEqualityChecker>{true, KeyComparator{1}, KeyEqualityChecker{1}};
        //new(t) BwTree<uint64_t, uint64_t, KeyComparator, KeyEqualityChecker>{true, KeyComparator{1}, KeyEqualityChecker{1}};
        t->UpdateThreadLocal(1);
        t->AssignGCID(0);
        std::atomic<int> next_thread_id;
...

        {
            ...
            auto func = [&]() {
                ...
                t->AssignGCID(thread_id);
                for (uint64_t i = start_key; i < end_key; i++) {
                        //cout << "i:" << i << " init_keys[i]:"<<init_keys[i]<<endl;
                        //if (i % 5000000 == 0) 
                            //std::cout << "Load phase: inserted " << i << " keys" << std::endl;
        ...
        std::cout << "wbtree loading initial keys" << std::endl;
#ifndef STRING_TYPE
        wbtree::tree *tree = wbtree::initTree();
        std::cout << "wbtree loading initial keys" << std::endl;
        {
            ...
            //    auto t = tree->getThreadInfo();
                ...
                for (uint64_t i = 0; i < LOAD_SIZE; i++) {
                    //tree->put((char *)init_keys[i]->fkey, init_keys[i]->value, t);
                    wbtree::Insert(tree, init_keys[i], &init_keys[i]);
                }
            //});
            auto duration = std::chrono::duration_cast<std::chrono::microseconds>(
                    std::chrono::system_clock::now() - starttime);
...

        {
            ...
            //    auto t = tree->getThreadInfo();
                for (uint64_t i = 0; i < LOAD_SIZE ; i++) {
                    if (ops[i] == OP_INSERT) {
                        wbtree::Insert(tree, keys[i], &keys[i]);
                    } else if (ops[i] == OP_READ) {
                        wbtree::Lookup(tree, keys[i]);
                        //uint64_t *ret = reinterpret_cast<uint64_t *> (tree->get((char *)keys[i]->fkey, t));
                        /*if (ap == UNIFORM && (uint64_t) ret != keys[i]->value) {
                            printf("[MASS] search key = %lu, search value = %lu\n", keys[i]->value, ret);
                            exit(1);
                        ...
                        uint64_t results[200];
                        //resultsFound = tree->scan((char *)keys[i]->fkey, ranges[i], results, t);
                        wbtree:Range_Lookup(tree, keys[i], ranges[i], results);
                    } else if (ops[i] == OP_DELETE) {
                        //tree->del((char *)keys[i]->fkey, t);
                        wbtree::Delete(tree, keys[i]);
                    } else if (ops[i] == OP_UPDATE) {
                        //tree->put((char *)keys[i]->fkey, keys[i]->value, t);
                        wbtree::Update(tree, keys[i], &keys[i]);
                    }
                }
            //});
            auto duration = std::chrono::duration_cast<std::chrono::microseconds>(
...
#ifndef STRING_TYPE
        fptree::tree * tree = fptree::initTree();
        //std::cout << "fptree starting" << std::endl;
        
        {
            ...
            auto starttime = std::chrono::system_clock::now();
            tbb::parallel_for(tbb::blocked_range<uint64_t>(0, LOAD_SIZE), [&](const tbb::blocked_range<uint64_t> &scope) {
                //std::cout << "FPTree Insert from " << scope.begin() << " to " << scope.end() << "LOAD_SIZE=" << LOAD_SIZE << std::endl;
                for (uint64_t i = scope.begin(); i != scope.end(); i++) {
                    
                    fptree::Insert(tree, init_keys[i], &init_keys[i]);
                }
            ...
            //    auto t = tree->getThreadInfo();
                for (uint64_t i = scope.begin(); i != scope.end(); i++) {
                    if (ops[i] == OP_INSERT) {
                        ...
                        //wbtree::Insert(tree, keys[i], &keys[i]);
                        fptree::Insert(tree, keys[i], &keys[i]);
                        
                    } else if (ops[i] == OP_READ) {
                        
                        fptree::Lookup(tree, keys[i]);
                        ...
                        fptree::Range_Lookup(tree, keys[i], ranges[i], results);
                        
                    } else if (ops[i] == OP_DELETE) {
                        //tree->del((char *)keys[i]->fkey, t);
                        //wbtree::Delete(tree, keys[i]);
                        
                        //fptree::Delete(tree, keys[i]);
                        
                    } else if (ops[i] == OP_UPDATE) {
                        //tree->put((char *)keys[i]->fkey, keys[i]->value, t);
                        //wbtree::Update(tree, keys[i], &keys[i]);
                        
                        fptree::Update(tree, keys[i], &keys[i]);
...
#endif
    } /*else if (index_type == TYPE_BZTREE) {
        std::cout << "check1" << std::endl;
        /*
        std::vector<char*> transInitKeys_keys;
        for (unsigned long long i = 0; i < LOAD_SIZE; i++) {
            char* buf = (char*)malloc(sizeof(uint64_t));
            snprintf(buf, sizeof(uint64_t), "%"PRIu64, init_keys[i]);
            transInitKeys_keys.push_back(buf);
        }*/

        /*
        bztree::BzTree* tree;
        
            constexpr uint32_t kDescriptorsPerThread = 1024;

            bztree::BzTree::ParameterSet param(3072, 1024, 4096);
            uint32_t num_threads = 16 + 1; // account for the loading thread
...

        {
            ...
            tbb::parallel_for(tbb::blocked_range<uint64_t>(0, LOAD_SIZE), [&](const tbb::blocked_range<uint64_t> &scope) {
                for (uint64_t i = scope.begin(); i != scope.end(); i++) {
                    std::cout << "BzTree Insert" << std::endl;
                    tree->Insert((char*) &init_keys[i], sizeof(init_keys[i]), init_keys[i]);
                }
            });
            */
            /*
            ...
            for (uint64_t i = 0; i < 1000; i++) {
                ...
                bztree->Insert((char*)&init_keys[i], sizeof(char), init_keys[i]);
            }
            auto duration = std::chrono::duration_cast<std::chrono::microseconds>(
                    std::chrono::system_clock::now() - starttime);
            printf("Throughput: load, %f ,ops/us\n", (LOAD_SIZE * 1.0) / duration.count());
...

        {
            ...
            tbb::parallel_for(tbb::blocked_range<uint64_t>(0, RUN_SIZE), [&](const tbb::blocked_range<uint64_t> &scope) {
                for (uint64_t i = scope.begin(); i != scope.end(); i++) {
                    if (ops[i] == OP_INSERT) {
                        bztree::ReturnCode rc_insert;
                        rc_insert = tree->Insert((char*) &keys[i], sizeof(keys[i]), keys[i]);
                    } else if (ops[i] == OP_READ) {
                        bztree::ReturnCode rc_read;
                        rc_read = tree->Read((char*) &init_keys[i], sizeof(init_keys[i]), &init_keys[i]);
                        if (rc_read.IsNotFound()) {
                            printf("NULL is found\n");
                        } else if (!rc_read.IsKeyExists()) {
                            printf("[BZTREE] wrong value is returned: <expected> %lu\n", keys[i]);
                            exit(1);
                        }
                    } else if (ops[i] == OP_SCAN) {
                        uint64_t buf[200];
                        int resultsFound = 0;
                        bztree::ReturnCode rc_scan;
                        tree->RangeScanBySize((char*) &keys[i], sizeof(keys[i]), ranges[i]);
                        
                    } else if (ops[i] == OP_UPDATE) {
                        bztree::ReturnCode rc_update;
                        rc_update = tree->Update((char*) &keys[i], sizeof(keys[i]), keys[i]);
                    }
                }
            });
            auto duration = std::chrono::duration_cast<std::chrono::microseconds>(
...
int main(int argc, char **argv) {
    ...
    else if (strcmp(argv[1], "bztree") == 0)
        index_type = TYPE_BZTREE;
    else {
