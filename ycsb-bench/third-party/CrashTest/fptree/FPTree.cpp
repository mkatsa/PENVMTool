...
//unsigned long long in_node_counter = 0;
//unsigned long long ln_node_counter = 0;
...
IN *allocINode()
{
	...
	//in_node_counter++;
	//printf(" in_node_counter:%ul", in_node_counter);
	node = new IN;
	node->type = THIS_IN;
	node->nKeys = 0;
	return node;
...
void Insert(tree *t, unsigned long key, void *value)
{
  ...
  while(true) {
	...
	if(should_split) {
		...
		//entry* valid_entry = new entry[NUM_LN_ENTRY];
...
void Range_Lookup(tree *t, unsigned char* start_key, int key_len, unsigned int num, 
		unsigned long buf[])
{
	unsigned long i, entry_num, loc, search_count = 0;
	void *curr = t->root;
	entry *sorted_entry = (entry*)malloc(NUM_LN_ENTRY * sizeof(entry));
	curr = find_leaf_node(curr, start_key, key_len);
	...
	while (curr != NULL) {
		...
		while (loc < NUM_LN_ENTRY) {
			loc = find_next_bit(&((LN*)curr)->bitmap, BITMAP_SIZE, loc);
			if (loc == BITMAP_SIZE)
				break;

			sorted_entry[entry_num] = ((LN*)curr)->entries[loc];
			loc++;
			entry_num++;
		}
		insertion_sort(sorted_entry, entry_num);
...
void *find_leaf_node(void *curr, unsigned char *key, int key_len) 
{
	...
	if (((LN *)curr)->type == THIS_LN) 
		return curr;
	//std::cout << "check 3_2" << std::endl;
	loc = Search((IN *)curr, key, key_len);
	//std::cout << "check 3_3" << std::endl;
	if (loc > ((IN *)curr)->nKeys - 1) 
...
void Insert(tree *t, unsigned char *key, int key_len, void *value)
{
    ...
    while (true) {
        ...
        /* Find proper leaf */
        curr = find_leaf_node(t->root, key, key_len);
        ...
        ((LN*)curr)->lock = 1;
        should_split = (((LN*)curr)->bitmap == IS_FULL);
	...
	if(should_split) {
		...
		split_LNode->pNext = ((LN*)curr)->pNext;

		for (j = 0; j < NUM_LN_ENTRY; j++)
			valid_entry[j] = ((LN*)curr)->entries[j];

		insertion_sort(valid_entry, NUM_LN_ENTRY);

		((LN*)curr)->bitmap = 0;
		for (j = 0; j < MIN_LN_ENTRIES; j++)
			insert_in_leaf_noflush(((LN*)curr), valid_entry[j].key,
					valid_entry[j].ptr);
		...
		if (memcmp((split_LNode->entries[0].key)->key, new_item->key, max((split_LNode->entries[0].key)->key_len, key_len)) > 0) {
			insert_in_leaf_noflush(((LN*)curr), new_item, value);
		} else
			insert_in_leaf_noflush(split_LNode, new_item, value);

		((LN*)curr)->pNext = split_LNode;
    ...
    speculative_lock.release();
		...
		insert_in_leaf(((LN*)curr), new_item, value);
	}
...
void *Update(tree *t, unsigned char* key, int key_len, void *value)
{
	...
	while (loc < NUM_LN_ENTRY) {
		...
		if (((LN*)curr)->fingerprints[loc] == hash(key, key_len) &&
				((LN*)curr)->entries[loc].key->key == key) {
			((LN*)curr)->entries[loc].ptr = value;
			flush_buffer(&((LN*)curr)->entries[loc].ptr, 8, true);
			return ((LN*)curr)->entries[loc].ptr;
		}
		loc++;
	}
