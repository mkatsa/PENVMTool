...
{
#ifdef LOGGING
	std::cout << "Range_Lookup() " << std::endl;
#endif
	int loc, i;
...
void Range_Lookup(tree *t, unsigned char* start_key, unsigned int key_len, unsigned int num, 
		unsigned long buf[])
{
	int loc, i;
	unsigned long search_count = 0;
...
//	clock_gettime(CLOCK_MONOTONIC, &t1);
	...
	loc = Search(curr, curr->slot, start_key, key_len);
//	printf("loc = %d\n", loc);
//	clock_gettime(CLOCK_MONOTONIC, &t2);
//	elapsed_time = (t2.tv_sec - t1.tv_sec) * 1000000000;
//	elapsed_time += (t2.tv_nsec - t1.tv_nsec);
...
int delete_in_leaf(node *curr, unsigned char* key, int key_len)
{
	int mid, j;
	...
	mid = Search(curr, curr->slot, key, key_len);
...
int Delete(tree *t, unsigned char* key, int key_len)
{
	int numEntries, errval = 0;
	node *curr = t->root;
...
//		errval = delete_in_leaf(curr, key);
	...
	errval = delete_in_leaf(curr, key, key_len);
