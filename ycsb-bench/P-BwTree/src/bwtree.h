...
//extern pthread_mutex_t debug_stop_mutex;
...
extern tbb::speculative_spin_mutex mutex;
...
class BwTree : public BwTreeBase {
  ...
  class AllocationMeta {
   ...
   public:
    ...
    AllocationMeta *GrowChunk() {
      ...
      //pthread_mutex_lock(&debug_stop_mutex);
    //tbb::speculative_spin_mutex::scoped_lock speculative_lock(mutex);
      //std::cout << "chuck_size:" << CHUNK_SIZE <<std::endl;
      //char *new_chunk = new char[CHUNK_SIZE];
    char* new_chunk = (char*)malloc(CHUNK_SIZE);
      //speculative_lock.release();
      
      //char* new_chunk = (char*)malloc(CHUNK_SIZE*sizeof(char));
      //pthread_mutex_unlock(&debug_stop_mutex);
      ...
      //std::cout << "2045" << std::endl;
      //speculative_lock.acquire(mutex);  
      //delete[] new_chunk;
      //free(new_chunk);
      //speculative_lock.release();
  ...
  class ElasticNode : public BaseNode {
    ...
    inline void PushBack(const ElementType *copy_start_p,
      ...
      if(std::is_trivially_copyable<KeyType>::value == false || 
        ...
        while(copy_start_p != copy_end_p) {
          //std::cout << "2300 before PushBack" << std::endl;
          PushBack(*copy_start_p);
          //std::cout << "2301 after PushBack" << std::endl;
          copy_start_p++; 
        }
      } else {
   ...
   public: 
    ...
    inline static ElasticNode *Get(int size,         // Number of elements
      ...
      //std::cout << "2341" << std::endl;
      //tbb::speculative_spin_mutex::scoped_lock speculative_lock(mutex);
      //char *alloc_base = \
        new char[sizeof(ElasticNode) + \
                   size * sizeof(ElementType) + \
                   AllocationMeta::CHUNK_SIZE];
      char *alloc_base = (char*)malloc(sizeof(ElasticNode) + size * sizeof(ElementType)+AllocationMeta::CHUNK_SIZE);
        // char[sizeof(ElasticNode) + \
                   size * sizeof(ElementType) + \
                   AllocationMeta::CHUNK_SIZE];                   
 ...
 public:
  ...
  void InitNodeLayout() {
    ...
    #endif
  //std::cout << "3117" << std::endl;
    root_node_p->PushBack(first_sep);
    //std::cout << "3119" << std::endl;
...
abort_traverse:
  ...
  InnerNode *CollectAllSepsOnInner(NodeSnapshot *snapshot_p,
    ...
    //std::cout << "4118" << std::endl;
    inner_node_p->PushBack(node_p->GetLowKeyPair());
    //std::cout << "4120" << std::endl;
        ...
        case NodeType::InnerType: {
          ...
          while(1) {
            ...
            if(sss_end_flag == true && array_end_flag == true) {
              ...
              while(sss.GetBegin() != sss_end_it) {
                ...
                if(data_node_type == NodeType::InnerInsertType) {
                  // Pop the value here
                  //std::cout << "4255" << std::endl;
                  new_inner_node_p->PushBack(sss.PopFront()->item);
                  //std::cout << "4257" << std::endl;
                } else {
            ...
            if(key_cmp_obj(copy_start_it->first, 
              ...
              //std::cout << "4274" << std::endl;
              new_inner_node_p->PushBack(*copy_start_it);
              //std::cout << "4276" << std::endl;
              ...
              if(data_node_type == NodeType::InnerInsertType) {
                // Pop the value here
                //std::cout << "4286" << std::endl;
                new_inner_node_p->PushBack(sss.PopFront()->item);
                //std::cout << "4288" << std::endl;
              } else {
                // This is possible
                // InnerNode: [2, 3, 4, 5]
              ...
              if(data_node_type == NodeType::InnerInsertType) {
                //std::cout << "4303" << std::endl;
                new_inner_node_p->PushBack(sss.PopFront()->item);
                //std::cout << "4305" << std::endl;
              } else {
          ...
          while(sss.GetBegin() != sss_end_it) {
            ...
            while(sss.GetFront()->GetIndexPair().first == current_index) {
              ...
              if(sss.GetFront()->GetType() == NodeType::LeafInsertType) {
                ...
                //std::cout << "5233" << std::endl;
                new_leaf_node_p->PushBack(sss.PopFront()->item);
                //std::cout << "5235" << std::endl;
              } else {
            ...
            //std::cout << "5255" << std::endl;
          } // while sss has not reached the copy end
          //std::cout << "5257" << std::endl;
	...
	if (pid == 0){
	  ...
	  ////std::cout << __func__ << ": ROOT SPLIT" << std::endl;
          ...
          if(ret == true) {
            ...
            //std::cout << "6526" << std::endl;
            //tbb::speculative_spin_mutex::scoped_lock speculative_lock(mutex);
            InnerRemoveNode *fake_remove_node_p = (InnerRemoveNode*) malloc(sizeof(InnerRemoveNode));
  ...
  void AdjustNodeSize(Context *context_p) {
	...
	// ############# ATOMIC 1 #################
        ...
        if(ret == true) {
          ...
          // is deleted
          //std::cout << "6914" << std::endl;
          //tbb::speculative_spin_mutex::scoped_lock speculative_lock(mutex);
          LeafRemoveNode *fake_remove_node_p = (LeafRemoveNode*)malloc(sizeof(LeafRemoveNode));
          //const LeafRemoveNode *fake_remove_node_p = \
            new LeafRemoveNode{new_node_id, new_leaf_node_p};
            new(fake_remove_node_p) LeafRemoveNode{new_node_id, 
                               new_leaf_node_p};
          //speculative_lock.release();
...
//std::cout << "6968" << std::endl;
//tbb::speculative_spin_mutex::scoped_lock speculative_lock(mutex);
        LeafRemoveNode *remove_node_p = (LeafRemoveNode*)malloc(sizeof(LeafRemoveNode));
...
//tbb::speculative_spin_mutex::scoped_lock speculative_lock(mutex);
      ...
      if(node_size >= INNER_NODE_SIZE_UPPER_THRESHOLD) {
        ...
        if(ret == true) {
          ...
          //std::cout << "7076" << std::endl;
          //tbb::speculative_spin_mutex::scoped_lock speculative_lock(mutex);
          InnerRemoveNode *fake_remove_node_p = (InnerRemoveNode*)malloc(sizeof(InnerAbortNode));
          //const InnerRemoveNode *fake_remove_node_p = \
...
//std::cout << "7137" << std::endl;
//tbb::speculative_spin_mutex::scoped_lock speculative_lock(mutex);
        InnerRemoveNode *remove_node_p = (InnerRemoveNode*)malloc(sizeof(InnerRemoveNode));
...
//std::cout << "7247" << std::endl;
//tbb::speculative_spin_mutex::scoped_lock speculative_lock(mutex);
InnerAbortNode *abort_node_p = (InnerAbortNode*)malloc(sizeof(InnerAbortNode));
    //InnerAbortNode *abort_node_p = \
      new InnerAbortNode{parent_node_p};
      new(abort_node_p_p) InnerAbortNode{parent_node_p};
//speculative_lock.release();
  ...
  class EpochManager {
   public:
    ...
    EpochManager(BwTree *p_tree_p) :
      tree_p{p_tree_p} {
        //std::cout << "8311" << std::endl;
        //tbb::speculative_spin_mutex::scoped_lock speculative_lock(mutex);
      current_epoch_p = (EpochNode*)malloc(sizeof(EpochNode));
...
//std::cout << "8439" << std::endl;
//tbb::speculative_spin_mutex::scoped_lock speculative_lock(mutex);
      EpochNode *epoch_node_p = (EpochNode*)malloc(sizeof(EpochNode));
...
//speculative_lock.release();
    ...
    void AddGarbageNode(const BaseNode *node_p) {
      ...
      //tbb::speculative_spin_mutex::scoped_lock speculative_lock(mutex);
      GarbageNode *garbage_node_p = (GarbageNode*)malloc(sizeof(GarbageNode));
...
try_join_again:
    ...
    inline void AddGarbageNode(const BaseNode *node_p) {
      //std::cout << "8615" << std::endl;
      tree_p->AddGarbageNode(node_p); 
      //std::cout << "8617" << std::endl;
      return;
    }
    ...
    void FreeEpochDeltaChain(const BaseNode *node_p) {
      ...
      while(1) {
        ...
        NodeType type = node_p->GetType();
        //tbb::speculative_spin_mutex::scoped_lock speculative_lock;
        switch(type) {
          case NodeType::LeafInsertType:
            next_node_p = ((LeafInsertNode *)node_p)->child_node_p;
...
//tbb::speculative_spin_mutex::scoped_lock speculative_lock(mutex);
     ...
     */
    void StartThread() {
      //std::cout << "8922" << std::endl;
      //tbb::speculative_spin_mutex::scoped_lock speculative_lock(mutex);
      thread_p = (std::thread*)malloc(sizeof(std::thread));
      new(thread_p) std::thread{[this](){this->ThreadFunc();}};
  ...
  void AddGarbageNode(const BaseNode *node_p) {
    ...
    if(GetCurrentGCMetaData()->node_count > GC_NODE_COUNT_THREADHOLD) {
      //std::cout << "9880" << std::endl;
      // Use current thread's gc id to perform GC
      PerformGC(gc_id);
      //std::cout << "9883" << std::endl;
    }
    //std::cout << "9881" << std::endl;
    return;
  }
  ...
  void PerformGC(int thread_id) {
    ...
    while(first_p != nullptr && \
      ...
      // Then free memory
      //std::cout << "9919" << std::endl;
      epoch_manager.FreeEpochDeltaChain((const BaseNode *)first_p->node_p);
      //std::cout << "9921" << std::endl;
      //std::cout << "delete first_p: " << first_p << std::endl;
      //tbb::speculative_spin_mutex::scoped_lock speculative_lock(mutex);
      //delete first_p;
      //speculative_lock.release();
      //std::cout << "after delete first_p: " << first_p << std::endl;
      //std::cout << "9923" << std::endl;
